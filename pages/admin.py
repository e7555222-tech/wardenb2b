...
tab_users, tab_leads = st.tabs(["👥 Kullanıcılar", "📋 Lead'ler"])
    ...
    try:
        ...
        if users_response.status_code == 200:
            ...
            if users:
                ...
                df_display = df[["id", "name", "email", "company", "is_admin", "created_at"]].copy()
                ...
                if selected_user:
                    ...
                    with col2:
                        tier_options = {"free": "🆓 Free", "pro": "💎 Pro", "enterprise": "🏢 Enterprise"}
                        new_tier = st.selectbox(
                            "Plan değiştir",
                            options=list(tier_options.keys()),
                            format_func=lambda x: tier_options[x],
                            key=f"tier_{selected_user['id']}",
                        )
                        if st.button("🔄 Planı Güncelle", use_container_width=True):
                            resp = requests.put(
                            ...
                            if resp.status_code == 200:
                                ...
                                st.error("❌ Plan güncellenemedi.")

                    with col3:
                        if st.button("🗑️ Kullanıcıyı Sil", use_container_width=True, type="primary"):
    ...
    try:
        ...
        if leads_response.status_code == 200:
            ...
            if leads:
                ...
                df_display = df[
                    ["id", "name", "email", "company_name", "budget", "score", "sentiment", "created_at"]
                ].copy()
